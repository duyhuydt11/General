...
// Timeout & Delay
#define TIME_OUT 								(SysTick->CTRL & (1<<16))
...
typedef enum
{
  ...
  Sys_Tick_100HZ        = 10U,
  Sys_Tick_1KHZ         = 1U,
  Sys_Tick_DEFAULT      = Sys_Tick_1KHZ
} Sys_Tick_FreqTypeDef;
...
extern Sys_Tick_FreqTypeDef uwTickFreq;
...
void Systick_Delay(uint32_t Delay, uint32_t Tickint);
void Systick_Timeout_Init(uint32_t Delay, uint32_t Tickint);
