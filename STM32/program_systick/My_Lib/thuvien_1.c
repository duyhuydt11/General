...
Sys_Tick_FreqTypeDef uwTickFreq = Sys_Tick_DEFAULT;
...
void GPIO_Config(void)
{
	...
	External_Interupt_Setup_Priority(3,0, EXTI4_IRQn);
...
// Systick Delay (ms)
void Systick_Timeout_Init(uint32_t Delay, uint32_t Tickint) //5ms
{
	// Maximum value of Delay => 2^24 - 1 = SystemClock*Delay;
	SysTick->LOAD = SystemCoreClock/1000*Delay; //1ms
	SysTick->VAL = 0;
	SysTick->CTRL = 5u | ((uint32_t)Tickint << 1);
}


//  Systick Delay no interupt (ms)
void Systick_Delay(uint32_t Delay, uint32_t Tickint) //5ms
{
	SysTick->LOAD = SystemCoreClock/1000; //1ms
	SysTick->VAL = 0;
	SysTick->CTRL = 5u | ((uint32_t)Tickint << 1);
	for(uint32_t x = 0; x < Delay; x++)
		while((SysTick->CTRL & (1<<16)) == 0); // 1ms
}

//  Systick Delay interupt init (ms)
void Systick_Delay_Interupt_Init(void)
{
	SysTick->LOAD = SystemCoreClock/1000; //1ms
	SysTick->VAL = 0;
	SysTick->CTRL = 5u | (1u << 1);
	External_Interupt_Setup_Priority(0,0, SysTick_IRQn);
}
...
// delay by systick interupt stm32
void Systick_Delay_Interupt(uint32_t Delay)
{
...
uint32_t Hold_Debounce_Button(My_GPIO_REG_TYPE* GPIOx, uint32_t GPIO_Pin)
{
	if((GPIOx->My_IDR & GPIO_Pin) == (uint32_t)GPIO_Pin_Reset)
	{
		...
		if((GPIOx->My_IDR & GPIO_Pin) == (uint32_t)GPIO_Pin_Reset)
		{
			Systick_Timeout_Init(2000, 0);
			while(((GPIOx->My_IDR & GPIO_Pin) == (uint32_t)GPIO_Pin_Reset) && TIME_OUT == 0);
